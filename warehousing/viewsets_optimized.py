...
class QuarantineWarehouseViewSet(viewsets.ModelViewSet):
    queryset = QuarantineWarehouse.objects.select_related(
        'part_number', 'item_code', 'qc_responsible', 'test_responsible'
    ).prefetch_related('category').all()
    serializer_class = QuarantineWarehouseSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['status', 'unit', 'destination']
...
class ProductWarehouseViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'retrieve':
            return ProductWarehouseDetailSerializer
        return ProductWarehouseSerializer
    ...
    def raw_materials(self, request, pk=None):
        ...
        raw_materials = product.raw_materials.all()[:20]  # محدود کردن تعداد
    ...
    def secondry_products(self, request, pk=None):
        ...
        product = self.get_object()
        secondry_products = product.secondry_products.all()[:20]  # محدود کردن تعداد
...
class ProductRawMaterialViewSet(viewsets.ModelViewSet):
    queryset = ProductRawMaterial.objects.select_related(
        'product', 'part_number', 'item_code'
    ).all()
    serializer_class = ProductRawMaterialSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['product', 'unit']
...
class SecondryWarehouseViewSet(viewsets.ModelViewSet):
    queryset = SecondryWarehouse.objects.all()
...
class SecondryWarehouseRawMaterialViewSet(viewsets.ModelViewSet):
    queryset = SecondryWarehouseRawMaterial.objects.select_related(
        'secondryWarehouse', 'part_number', 'item_code'
    ).all()
    serializer_class = SecondryWarehouseRawMaterialSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['secondryWarehouse', 'unit']
    search_fields = ['raw_material_name', 'user_who_used', 'serial_number', 'item_code__product_code', 'part_number__product_part']
    ordering_fields = ['raw_material_entry_date', 'raw_material_name', 'quantity', 'id']
    ordering = ['-raw_material_entry_date']
...
class ProductDeliveryViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'retrieve':
            return ProductDeliveryDetailSerializer
        return ProductDeliverySerializer
    ...
    def items(self, request, pk=None):
        ...
        result = {
            'products': ProductDeliveryProductSerializer(delivery.product_items.all()[:20], many=True).data,
            'secondry_products': ProductDeliverySecondryProductSerializer(delivery.secondry_items.all()[:20], many=True).data,
            'raw_materials': ProductDeliveryRawMaterialSerializer(delivery.raw_material_items.all()[:20], many=True).data,
        }
...
class ProductDeliveryRawMaterialViewSet(viewsets.ModelViewSet):
    queryset = ProductDeliveryRawMaterial.objects.select_related(
        'delivery', 'part_number', 'item_code'
    ).all()
    serializer_class = ProductDeliveryRawMaterialSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['delivery', 'unit']
    search_fields = ['raw_material_name', 'user_who_used', 'serial_number', 'item_code__product_code', 'part_number__product_part']
    ordering_fields = ['delivery_date', 'quantity', 'id']
    ordering = ['-delivery_date']
...
class ExternalProductDeliveryViewSet(viewsets.ModelViewSet):
    queryset = ExternalProductDelivery.objects.all()
    serializer_class = ExternalProductDeliverySerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['reference_letter']
    ...
    def items(self, request, pk=None):
        ...
        delivery = self.get_object()
        result = {
            'products': ExternalProductDeliveryProductSerializer(delivery.product_items.all()[:20], many=True).data,
            'secondry_products': ExternalProductDeliverySecondryProductSerializer(delivery.secondry_items.all()[:20], many=True).data,
            'raw_materials': ExternalProductDeliveryRawMaterialSerializer(delivery.raw_material_items.all()[:20], many=True).data,
        }
        return Response(result)
...
class ReturnedFromCustomerViewSet(viewsets.ModelViewSet):
    queryset = ReturnedFromCustomer.objects.all()
